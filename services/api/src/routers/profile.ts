import { z } from 'zod';
import { router, publicProcedure, protectedProcedure, profileProcedure } from '../trpc';
...
const ProfileTypeSchema = z.enum(['MUSICIAN', 'VENUE', 'PROMOTER', 'VISUAL_ARTIST', 'BAND']);
...
export const profileRouter = router({
  ...
  create: protectedProcedure
    .input(
      z.object({
        handle: z.string().min(2).max(30).regex(/^[a-z0-9-]+$/, 'Lowercase letters, numbers and hyphens only'),
  ...
  update: profileProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (ctx.activeProfile.role === 'MEMBER') {
        throw new TRPCError({ code: 'FORBIDDEN', message: 'Members cannot edit profile details' });
      }
  ...
  follow: profileProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await ctx.prisma.follow.create({
        data: { followerId: ctx.activeProfile.id, followingId: input.profileId },
      });
  ...
  unfollow: profileProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      await ctx.prisma.follow.deleteMany({
        where: { followerId: ctx.activeProfile.id, followingId: input.profileId },
      });
  ...
  addMember: profileProcedure
    .input(z.object({ accountId: z.string(), role: z.enum(['ADMIN', 'MEMBER']) }))
    .mutation(async ({ input, ctx }) => {
      ...
      const existing = await ctx.prisma.profileMember.findUnique({
        where: { accountId_profileId: { accountId: input.accountId, profileId: ctx.activeProfile.id } },
      });

      if (existing) {
        throw new TRPCError({ code: 'CONFLICT', message: 'Account is already a member' });
      }

      await ctx.prisma.profileMember.create({
        data: { accountId: input.accountId, profileId: ctx.activeProfile.id, role: input.role },
      });
