...
import { SuiStakeProvider } from './providers/sui-stake';
...
const providers: Record<string, IdentityProvider> = {
  ...
  sui_stake: new SuiStakeProvider(),
};

export function getProvider(name: string): IdentityProvider {
  const provider = providers[name];
  if (!provider) {
    throw new Error(`Unknown identity provider: "${name}". Available: ${Object.keys(providers).join(', ')}`);
  }
  return provider;
}
